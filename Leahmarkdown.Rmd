---
title: "MUSA508-Leah-Palak"
author: "Leah Shapiro & Palak Agarwal"
...
Many of the features in this model were enginereed using two functions: **Nearest neighbor** and **Multiple ring buffer.** The nearest neighbor function finds the average distance from the measuring feature to the measured feature. The function requires 3 input variables - the dependent feature to measure from, the features to measure to, and the number of features. For example, a nearest neighbor feature could measure the average distance from each house to its three closest public parks. 

The multiple ring buffer creates concentric rings around point features. This can be used, for example, to determine how many crimes have occured in the last year within a .5 mile radius of each house.
...
Proximity to public services and amenities can add value to houses. We considered the following public services and amenities when building our model:
* **Transit Stops**        
...
```{r Public_services, message=FALSE,warning=FALSE}
# Transit Data 
...
```{r Spatial_Structure, message=FALSE, warning=FALSE}
...
```{r num_internal, message=FALSE, warning=FALSE}
...
```{r cat_internal, message=FALSE, warning=FALSE}
...
```{r ammenities, message=FALSE, warning=FALSE}
...
The presence of highly correlatd, or colinear, variables in a model can lead to unwanted redundancy. We used the correlation matrix below to determine which variables are colinear.
```{r corr_matrix, message=FALSE, warning=FALSE}
numericVars1 <-

  select_if(st_drop_geometry(MiamiDF), is.numeric) %>% na.omit() %>%

  select(SalePrice,LivingSqFt,EffectiveYearBuilt,ActualSqFt,LotSize,Bath,Stories,Bed)
...
numericVars2 <-
  ...
  select(SalePrice,dist.metro,bar_nn2,school_nn1,worship_nn1,parking_nn2,office_nn3,hospital_nn1)
...
numericVars3 <-
  ...
  select(SalePrice,logCoastDist,lagPrice,MedRent,MedHHInc,pctWhite,pctHispanic)
...
numericVars4 <-
  ...
  select(SalePrice,Docks, LuxuryPool, Whirpool, `XFs_Elevator - Passenger`, Fence, `8ftres3to8ftPool`,`2to4ftPool`,
...
To help us identify which variables might be most important to include in our model, we generated the following scatter plots visualizing the correlation between variables and home sale prices.
...
```{r var_map2, message=FALSE, warning=FALSE}
ggplot(Miamitracts)+
...
### Metro Stations Map
```{r var_map3, message=FALSE, warning=FALSE}
ggplot() +
...
If we filter out houses valued over $3,000,000, the MAE of the model improves. 

```{r filter5mil message=FALSE, warning=FALSE}
Miami.test <-
  Miami.test %>%
  mutate(SalePrice.Predict = predict(reg, Miami.test),
         SalePrice.Error = SalePrice.Predict - SalePrice,
...
The first plot shows thatas the price of a house increases, the prices of nearby houses also increase. This demonstrates the importance of including spatial features in our model.
...
Moran's I provides another means of determing whether our errors are spatially correlated. Moran's I here is positive, suggesting positive spatial autocorrelation in our model.
