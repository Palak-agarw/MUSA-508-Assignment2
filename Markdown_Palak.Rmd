...
For this assignment we were asked to build a better predictive model of home prices for **Miami-Dade County, Florida** using internal characteristics, amenities/public services or spatial structures.
...
For this project, we used two main functions : **Nearest neighbor** and **Multiple ring buffer.** The nearest neighbor function found the average distance from the measuring feature to the measured feature. The function requires 3 input variables - the dependent feature to measure from, the features to measure to and the number of features. The multiple ring buffer creates concentric rings around the point feature and finds the features that fall in buffer. 
...
First lets start with internal characteristics of the house. For this we looked at the various extra factors of the houses, number of beds, baths, living sq ft, actual sq ft, lot size, year built and stories. Some characteristics like number of beds and year built have been converted to categorical feature as that makes more sense cause of the number of combinations possible and the actual value does not matter as much.
...
```
### Converting it into spatial feature and adding neighborhoods
...
While one is looking for houses, proximity to public services and amenities is one major factor for selection. Many factors can be considered, below are some of the amenities we looked in  :     
* **Transit Data** : Miami does not have an extensive metro rail system and hence our assumption is that it would be more sought after.         
...
House sales are affected by spatial features ......    
* **Neighborhood**      
* **Shoreline**     
* **Sale Price around**  
* **Median Rent** 
...
#add to MiamiDF and convert to miles
MiamiDF <-
  MiamiDF %>%  
...
MiamiDF$pctWhite <- ifelse(is.na(MiamiDF$pctWhite), 90, MiamiDF$pctWhite) 
MiamiDF$pctHispanic <- ifelse(is.na(MiamiDF$pctHispanic), 90, MiamiDF$pctHispanic) 
